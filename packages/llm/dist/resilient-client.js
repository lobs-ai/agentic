...
export class ResilientLLMClient {
    ...
    clientConfig;
    attemptLog = [];
    constructor(primaryModel, options = {}, clientConfig) {
        ...
        for (const modelString of this.models) {
            ...
            for (let attempt = 0; attempt < this.maxRetries; attempt++) {
                ...
                try {
                    ...
                    console.warn(`[resilient-client] ${modelString} attempt ${attempt + 1} failed: ${classified.message}. ` +
                        `Retrying in ${Math.round(delay)}ms...`);
                    await sleep(delay);
                }
            }
            console.error(`[resilient-client] All ${this.maxRetries} attempts failed for ${modelString}. ` +
 ...
 * ```
 */
export function createResilientClient(primaryModel, options, config) {
