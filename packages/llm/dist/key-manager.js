...
export class KeyManager {
     ...
     */
    getKeySelection(provider, sessionId) {
        const pool = this.pools.get(provider);
        if (!pool || pool.keys.length === 0)
            return null;
        const keyIndex = this._assignKey(pool, sessionId);
     ...
     */
    markFailed(provider, keyIndex, reason, failureType, cooldownMs) {
        const key = this._getKey(provider, keyIndex);
        if (!key)
            return;
        key.healthy = false;
        ...
        if (failureType === "auth") {
            ...
            const defaultCooldown = failureType === "rate_limit" ? 15 * 60_000 : 10 * 60_000;
     ...
     */
    markSessionFailed(provider, sessionId, reason, failureType, cooldownMs) {
        const pool = this.pools.get(provider);
        if (!pool)
            return;
     ...
     */
    rotateSession(provider, sessionId, reason) {
        const pool = this.pools.get(provider);
        if (!pool)
            return false;
     ...
     */
    trackRequestStart(provider, keyIndex) {
        const key = this._getKey(provider, keyIndex);
        if (key)
            key.inFlightCount++;
    }
     ...
     */
    trackRequestEnd(provider, keyIndex, _success) {
        const key = this._getKey(provider, keyIndex);
        if (key)
            key.inFlightCount = Math.max(0, key.inFlightCount - 1);
    }
    /**
     * Get a health summary for a provider pool.
     */
    getPoolHealthSummary(provider) {
        const pool = this.pools.get(provider);
        if (!pool)
            return { total: 0, healthy: 0, authFailed: 0, rateLimited: 0, providerFailed: 0 };
        const now = Date.now();
        let healthy = 0;
        let authFailed = 0;
        let rateLimited = 0;
    ...
    // ── Internal ───────────────────────────────────────────────────────────────
    _assignKey(pool, sessionId, excludeIndex) {
    ...
    _isHealthy(key) {
        if (key.authFailed)
            return false;
        if (key.cooldownUntil && key.cooldownUntil > Date.now())
            return false;
        return true;
    }
    _getKey(provider, keyIndex) {
        const pool = this.pools.get(provider);
        if (!pool)
            return null;
 ...
 * ```
 */
export function configureKeyManager(config) {
