...
import { buildCompatibleClient, KNOWN_ENDPOINTS, } from "./providers/openai-compatible.js";
// ── parseModelString ──────────────────────────────────────────────────────────
/**
...
export function parseModelString(model) {
    ...
    if (slashIdx === -1) {
        throw new Error(`Invalid model string "${model}": must be "provider/model-id". ` +
    ...
    if (!knownProviders.includes(providerRaw)) {
        throw new Error(`Unknown provider "${providerRaw}" in model string "${model}". ` +
 ...
 * ```
 */
export function createClient(model, config) {
    const { provider, modelId } = parseModelString(model);
    // Helper: get first key for a provider from config
    const getKey = (providerName) => config?.keys?.[providerName]?.keys?.[0]?.key;
    // Helper: get base URL for a provider
    const getBaseUrl = (providerName) => config?.baseUrls?.[providerName];
    switch (provider) {
        case "anthropic": {
        ...
        case "openrouter": {
            const apiKey = getKey("openrouter") ?? process.env.OPENROUTER_API_KEY;
        ...
        case "openai-compatible": {
            ...
            const baseURL = getBaseUrl("openai-compatible") ??
