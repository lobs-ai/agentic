...
export const grepToolDefinition = {
    name: "grep",
    description: "A ripgrep-powered search tool for file contents. Use this instead of invoking grep or rg through Bash. " +
    ...
    input_schema: {
        ...
        properties: {
            ...
            case_sensitive: {
                type: "boolean",
                description: "Case sensitive search (default: smart case — case sensitive if pattern has uppercase)",
            },
            max_results: {
                type: "number",
...
function relativizePaths(output, cwd) {
    const prefix = cwd.endsWith("/") ? cwd : cwd + "/";
    return output.replaceAll(prefix, "");
}
function applyMaxResults(output, maxResults) {
...
// ── Tool Implementation ──────────────────────────────────────────────────────
export async function grepTool(params, cwd) {
    ...
    const contextLines = typeof params.context_lines === "number" ? params.context_lines : 0;
    const caseSensitive = params.case_sensitive;
    const maxResults = typeof params.max_results === "number" ? params.max_results : DEFAULT_MAX_RESULTS;
    ...
    return new Promise((resolvePromise) => {
        ...
        child.stdout.on("data", (d) => {
            if (output.length < MAX_CAPTURE)
                output += d.toString().slice(0, MAX_CAPTURE - output.length);
        });
        child.stderr.on("data", (d) => {
            if (stderr.length < MAX_CAPTURE)
                stderr += d.toString().slice(0, MAX_CAPTURE - stderr.length);
        });
        child.on("error", (err) => {
            clearTimeout(timer);
