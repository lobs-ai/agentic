...
export const readToolDefinition = {
    name: "read",
    description: "Reads a file from the local filesystem. Assume any user-provided path is worth checking. " +
 ...
 */
export function createReadSnapshot(filePath) {
    try {
 ...
 */
export function hasRecentlyReadFile(filePath) {
 ...
 */
export function updateReadSnapshot(filePath) {
    createReadSnapshot(filePath);
}
/**
 * Get the read snapshot for a file (used by edit to check staleness).
 */
export function getReadSnapshot(filePath) {
    return readSnapshots.get(filePath);
}
// ── Tool Implementation ──────────────────────────────────────────────────────
export async function readTool(params, cwd) {
    ...
    const limit = typeof params.limit === "number" ? params.limit : DEFAULT_READ_LINES;
    ...
    if (numbered.length <= DEFAULT_READ_CHARS)
        return numbered;
