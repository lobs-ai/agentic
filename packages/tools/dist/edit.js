...
export const editToolDefinition = {
    name: "edit",
    description: "Performs exact string replacements in files. You must use Read on the file before editing it. " +
 ...
 */
function computeDiff(before, after, filePath) {
    ...
    while (i < maxLen) {
        ...
        if (bLine !== aLine) {
            let j = i;
            while (j < maxLen && (beforeLines[j] ?? "") !== (afterLines[j] ?? ""))
                j++;
...
function applyEdit(content, op) {
    ...
    if (!replace_all && count > 1) {
        throw new Error(`old_string is ambiguous — found ${count} occurrences. ` +
...
// ── Tool Implementation ──────────────────────────────────────────────────────
export async function editTool(params, cwd) {
