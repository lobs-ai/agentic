...
function parseJson(path: string): { valid: boolean; data?: unknown; error?: string } {
  if (!existsSync(path)) {
...
function validateModelsConfig(path: string): ValidationResult {
  ...
  if (data.agents !== undefined) {
    if (typeof data.agents !== "object" || data.agents === null) {
      ...
      for (const [agentType, config] of Object.entries(data.agents as Record<string, unknown>)) {
...
function validateIdentityConfig(path: string): ValidationResult {
  const result: ValidationResult = {
    file: "identity.json",
    valid: true,
    errors: [],
  ...
  for (const section of ["bot", "owner"] as const) {
    if (data[section] !== undefined) {
      const s = data[section] as Record<string, unknown>;
...
function validateKeysConfig(path: string, isLegacy: boolean): ValidationResult {
  ...
  if (isLegacy) {
    result.warnings.push(
      "Using legacy location — move secrets to secrets/keys.json and gitignore that directory"
    );
  }
  ...
  for (const [pool, keys] of Object.entries(data as Record<string, unknown>)) {
    // Support both array format and { keys: [...] } format
    const keyList: unknown[] | null = Array.isArray(keys)
      ? keys
      : (keys && typeof keys === "object" && Array.isArray((keys as Record<string, unknown>).keys))
        ? (keys as Record<string, unknown[]>).keys
        : null;

    if (keyList === null) {
      result.errors.push(
        `Provider '${pool}' must be an array of keys or { keys: [...], strategy: "sticky-failover" }`
      );
      result.valid = false;
 ...
 */
export function validateAllConfigs(configDir?: string): AllConfigsResult {
 ...
 */
export function validateModelsObject(data: unknown): ValidationResult {
  const result: ValidationResult = {
    file: "models (object)",
    valid: true,
    errors: [],
