 ...
 */
export function getSecretsDir(configDir?: string): string {
 ...
 */
export function loadRuntimeConfig(opts?: LoadConfigOptions): RuntimeConfig {
  const configPath =
 ...
 */
export function getServerPort(defaultPort = 9420, opts?: LoadConfigOptions): number {
 ...
 */
export function getGatewayConfig(opts?: LoadConfigOptions): { port: number; token: string } {
  const config = loadRuntimeConfig(opts);
 ...
 */
export function loadConfigFile<T = unknown>(
  filename: string,
