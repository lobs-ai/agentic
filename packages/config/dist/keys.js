...
function normalizeKeyEntries(entries) {
    ...
    return normalized.filter((entry) => entry !== undefined);
}
function dedupeKeyEntries(entries, provider) {
    const seen = new Set();
    const deduped = [];
    ...
    if (provider && deduped.length !== entries.length) {
        ...
        console.warn(`[config/keys] Removed ${removed} duplicate ${provider} key entr${removed === 1 ? "y" : "ies"}`);
    }
    return deduped;
}
function normalizePool(pool) {
    // Current format: { keys: [...], strategy: "sticky-failover" }
    if (pool && typeof pool === "object" && Array.isArray(pool.keys)) {
        const keys = dedupeKeyEntries(normalizeKeyEntries(pool.keys));
        if (keys.length === 0)
            return undefined;
 ...
 */
export function normalizeKeyConfig(data) {
...
// ── File loading ──────────────────────────────────────────────────────────────
function loadConfigFile(configDir, silent = false) {
    ...
    if (existsSync(newPath)) {
        try {
            ...
            if (!silent)
                console.warn(`[config/keys] Failed to load ${newPath}:`, err);
            return undefined;
        }
    }
    if (existsSync(legacyPath)) {
        ...
        try {
            ...
            if (!silent)
                console.warn(`[config/keys] Failed to load ${legacyPath}:`, err);
            return undefined;
        }
    }
    return undefined;
}
function parseEnvKeys(envVar) {
 ...
 */
export function getFirstKey(provider, opts) {
 ...
 */
export function hasKeyForProvider(provider, opts) {
 ...
 */
export function buildKeyConfig(input) {
    const config = {};
